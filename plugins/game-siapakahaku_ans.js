...
handler.before = async function (m) {
    ...
    if (!(id in this.siapakahaku)) return m.reply('el asulto has terminado')
    if (m.quoted.id == this.siapakahaku[id][0].id) {
        let json = JSON.parse(JSON.stringify(this.siapakahaku[id][1]))
        if (['.who', 'ayuda', ''].includes(m.text)) return !0
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            global.db.data.users[m.sender].exp += this.siapakahaku[id][2]
            await this.sendBut(m.chat, `*derechar!* +${this.siapakahaku[id][2]} XP`, wm, 'quien soy', '.quien soy')
