...
handler.before = async function (m) {
  ...
  if (!(id in this.tebakkabupaten)) return m.reply('el asulto has terminado')
  if (m.quoted.id == this.tebakkabupaten[id][0].id) {
    let json = JSON.parse(JSON.stringify(this.tebakkabupaten[id][1]))
    if (['.tebu', 'ayuda', ''].includes(m.text)) return !0
    if (m.text.toLowerCase() == json.title.toLowerCase()) {
      global.db.data.users[m.sender].exp += this.tebakkabupaten[id][2]
      await this.sendBut(m.chat, `*cierto!* +${this.tebakkabupaten[id][2]} XP`, '', 'Adivina el Distrito', '.tebakkabupaten', m)
      clearTimeout(this.tebakkabupaten[id][3])
      delete this.tebakkabupaten[id]
    } else if (similarity(m.text.toLowerCase(), json.title.toLowerCase().trim()) >= threshold) m.reply(`*un poco mas!*`)
    else m.reply(`*falso!*`)
  }
  return !0
}
