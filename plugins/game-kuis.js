...
let handler  = async (m, { conn, usedPrefix }) => {
  ...
  let id = m.chat
  if (id in global.kuis) return conn.reply(m.chat, 'todavia hay pregunta sin responder el este chat', global.kuis[id][0])
  global.kuis[id] = [
    await conn.reply(m.chat,`「 uso mal es 」\n\nejemplo :\n${pickRandom(global.kuis)}\n\ntiempo : 30.00 segundo\nBonos : 5000 XP`, m),
    kuis, 4,
    setTimeout(() => {
      if (global.kuis[id]) conn.sendBut(m.chat, `se acabo el tiempo!\n\n intentelos de nuevo la próxima`, wm, 'Kuis', '.kuis', m)
